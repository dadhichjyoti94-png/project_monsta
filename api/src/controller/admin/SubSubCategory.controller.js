...
console.log("SUB CATEGORY BODY =", request.body);
    var sorting = {
        _id: "desc"
    }
...
if (orCondition.length > 0) {
    ...
    SubCategoryModel.find(filter).select('name').sort(sorting)
        ...
        .catch((error) => {
             console.log("SUB CATEGORY BACKEND ERROR =", error);
...
exports.create = async (request, response) => {
    ...
    if (request.body) {
        if (request.body.slug == undefined || request.body.slug == "") {
            ...
            dataSave.slug = await generateUniqueSlug(SubSubCategoryModel, slug);
    ...
    SubSubCategoryModel(dataSave).save()
        .then((result) => {
            const data = {
                _status: true,
...
exports.view = async (request, response) => {
    ...
    var totalRecords = await SubSubCategoryModel.find(filter).countDocuments();

   SubSubCategoryModel.find(filter)
    .select('name parent_category_id slug image status order sub_category_id')
...
exports.update = async (request, response) => {
    ...
    var getDetails = await SubSubCategoryModel.findOne({ _id: request.params.id });
