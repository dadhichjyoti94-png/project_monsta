...
const  colourModel =require('../../modles/colour')

exports.create = async (request, response) => {
    // datasave varbale m keys ko save kara diya
    const dataSave = request.body;

    // insert ki query

    colourModel(dataSave).save()
        .then((result) => {
            const data = {
                _status: true,
...
exports.view = async (request, response) => {
    ...
    //NAME SE MATCH 
        ...
        if(request.body.color_code!= undefined && request.body.color_code != ''){
            var colorCodeRegex = new RegExp("^" + request.body.color_code ,'i')
            andCondition.push({ color_code : colorCodeRegex})
        }
    ...
    var totalRecords = await colourModel.find(filter).countDocuments();

    colourModel.find(filter).select('name status color_code order').sort(sorting).limit(limit).skip(skip)
        .then((result) => {
            if (result.length > 0) {
                const data = {
                    _status: true,
